...
Remote videos stream through `AVURLAsset`/`AVPlayerItem` instead of downloading the complete file before playback. Readiness checks use `GET` with `Range: bytes=0-1` and accept `200` or `206`, avoiding CDN incompatibilities with `HEAD`. The SDK retries transient CDN readiness responses for up to approximately 15.5 seconds, then keeps the preview open with a clear Retry button.

The chat bearer token is attached only to media URLs whose scheme, host, and effective port match the configured InstaChat API origin. Direct CDN and presigned image, audio, video, and file URLs omit `Authorization`. The same policy covers downloads, retries, Range probes, and `AVURLAsset` headers, while same-origin API media proxies remain authenticated.
...
Received media that remains unavailable after the bounded retry window indicates that the backend published its attachment before the CDN object was readable. The backend should verify a CDN `GET` or Range request returns `200`/`206` before broadcasting the media message.
